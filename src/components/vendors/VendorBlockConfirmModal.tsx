      ...
      <div className="flex flex-col gap-4">
        <div className="flex items-center gap-3 p-3.5 bg-[#FAF9F6] border border-[#E4DCC9] rounded-xl">
          ...
          <p className="text-xs text-[#18281F] leading-relaxed">
            {isCurrentlyBlocked ? (
              <>
                Reactivating <strong className="text-[#18281F]">{vendor.storeName}</strong> will allow the vendor store to resume accepting orders across residential societies.
              </>
            ) : (
              <>
        ...
        {!isCurrentlyBlocked && (
          <div className="flex flex-col gap-1.5">
            <label className="text-xs font-bold text-[#18281F] uppercase tracking-wider flex items-center gap-1.5">
              <MessageSquare size={14} className="text-[#C4A066]" /> Custom Block Reason Message (Sent to Vendor)
            </label>
            <textarea
